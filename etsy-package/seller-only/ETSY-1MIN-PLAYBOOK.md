...
- [ ] BBC micro:bit V2 (optional) powered on, showing the **X** icon *(if not → re-flash `makecode.ts`)*
...
- [ ] Desk cleared — only the board + one prop (buggy / emotion / cable) in frame
...
## Script Template A — "Talking Robot — 13 Emotions · 16 Voices (incl. 8 Arabic regionals) · TTS/STT · Optional micro:bit Mirror" (hero demo)
...
|---|---|---|
| 0–3 s | *"Talking Robot — 13 Emotions · 16 Voices (incl. 8 Arabic regionals) · TTS/STT · Optional micro:bit Mirror — in the browser, in 60 seconds."* | Close-up of the real board with the X icon on the LEDs. Your hand holds it. |
...
| 3–10 s | *"One browser tab. Live sensors. No install on 30 laptops."* | Click Connect. Status goes green. Cut to Emotions panel — values streaming. |
| 10–20 s | *"Calibrate the mouth anim right in the UI. Baseline the sound for a quiet classroom."* | Emotions panel → Calibrate. Show the Set Level / Set Ambient buttons. |
| 20–30 s | *"Record a full lesson. Export CSV. Drop into a lab report."* | Speech panel → Record → Stop → Save Session → CSV. |
...
| 3–10 s | *"The micro:bit drives the emotion. The 3D model mirrors the real robot."* | Tab 6 (animated eyes). Real robot next to the laptop. Robot in 3D follows. |
| 10–20 s | *"D-pad in the browser → arrows on the LED → commands to the motors."* | Tab 4 (Voice Input). Press ⬆ → LED arrow. Robot crawls forward. |
...
- Use the listing **title** to echo the video's first words. If you say *"Talking Robot — 13 Emotions · 16 Voices (incl. 8 Arabic regionals) · TTS/STT · Optional micro:bit Mirror"*, the listing title starts with the same opening words.
...
DURING  Tilt → 3D · A → LED · slider → emotion · Simulate → graph
...
**Title (140 chars max):**
```
Talking Robot — BLE Control Panel for Teachers & Kids · Sensors · Emotion · Live Graph · 3D Models · No Install · Chrome / Edge
```

**160-char short description:**
```
Browser product V2 (optional). Sensors, emotions, live graphs, 3D models, and a Voice Input — all from Chrome. No install, no account.
```

**13 Etsy tags:**
```
micro:bit, BLE, bluetooth, education, STEM, robotics, kids coding, teacher resource, sensors, emotions, classroom, home school, makerspace
```
...
2. **Tabs overview** — grid screenshot of all 13 emotions with emoji labels.
...
5. **Hand-holding** — real emotion plugged into P1, sliders on the screen.
...
*Made for Talking Robot v1.0.0 — print this on A4, laminate, keep next to the shoot rig.*
