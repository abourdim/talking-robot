 ...
 * micro:bit sensor sessions (Pick an emotion, tilt, mouth anim, etc.).
 *
 ...
 *
 * Each CSV matches the format produced by the app's Speech panel CSV export:
 ...
 *   - mouth anim-walk-north     (heading ~10° drifting)
...
const SCENARIOS = {
  ...
  'mouth anim-walk-north': ({ duration_s = 60, hz = 5 }) => {
    const out = [];
    let h = 8 + gauss(0, 3);
    for (let i = 0; i < duration_s * hz; i++) {
      const t = i * (1000 / hz);
      ...
      if (h < 0) h += 360;
      if (h >= 360) h -= 360;
      out.push([t, 'mouth anim', +h.toFixed(1)]);
    }
    return out;
  },
...
const plan = [
  ...
  { scenario: 'mouth anim-walk-north', n: 9 },
...
for (const { scenario, n } of plan) {
  for (let i = 1; i <= n; i++) {
    ...
    if (scenario === '5-second-bursts') {
      extras.sensor = ['temp', 'light', 'sound', 'mouth anim'][i % 4];
    }
...
| \`mouth anim-walk-north\` | ${plan.find(p => p.scenario === 'mouth anim-walk-north').n} | Circular data, drift analysis |
