...
- [ ] `etsy-package/tools/generate-video.mjs` — ffmpeg-based generator
   ...
   `Talking Robot` / `Talk • Listen • Emote` substitutions don't fit your design.
...
- [ ] (Optional) `gh api -X POST "repos/abourdim/talking-robot/pages" -f 'source[branch]=main' -f 'source[path]=/'` — enable GitHub Pages so the live demo URL in the listing actually works
